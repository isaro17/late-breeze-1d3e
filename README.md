...
> [!IMPORTANT]
> When using C3 to create this project, select "no" when it asks if you want to deploy. You need to follow this project's [setup steps](https://github.com/cloudflare/templates/tree/main/late-breeze-1d3e#setup-steps) before deploying.
...
```bash
npm create cloudflare@latest -- --template=cloudflare/templates/late-breeze-1d3e
```

A live public deployment of this template is available at [https://late-breeze-1d3e.templates.workers.dev](https://late-breeze-1d3e.templates.workers.dev)
...
1. Install the project dependencies with a package manager of your choice:
   ...
   npm install
   ```
2. Create a [D1 database](https://developers.cloudflare.com/d1/get-started/) with the name "autumn-leaf-aa70-d1":
   ```bash
   npx wrangler d1 create autumn-leaf-aa70-d1
   ```
   ...and update the `database_id` field in `wrangler.json` with the new database ID.
3. Run the following db migration to initialize the database (notice the `migrations` directory in this project):
   ```bash
   npx wrangler d1 migrations apply --remote autumn-leaf-aa70-d1
   ```
